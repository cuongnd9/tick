...
const DatePickerList: React.FC<Props> = ({
  initialValue,
  onGetDateList,
  reset
}) => {
  const [dueDate, setDueDate] = useState(new Date(initialValue.dueDate));
  const [reminderDate, setReminderDate] = useState(
    new Date(initialValue.reminderDate)
  );
  const [showDueDate, setShowDueDate] = useState(false);
  const [showReminderDate, setShowReminderDate] = useState(false);

  useEffect(() => {
    setDueDate(new Date(initialValue.dueDate));
    setReminderDate(new Date(initialValue.reminderDate));
  }, [reset]);
  return (
